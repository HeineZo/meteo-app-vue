		...
		<div class="mx-auto px-4 sm:px-6 lg:px-8">
			<div class="flex h-20 items-center justify-between gap-10">
					...
					<div class="flex-shrink-0">
						<a href="/">
							<img
								...
								alt="Logo de l'application météo" />
						</a>
					</div>
					<!-- Navigation -->
					<div class="hidden sm:block">
						<div class="flex ml-10 items-baseline gap-5">
							<RouterLink
								...
								:class="{
									'bg-primary-400':
										location.name === 'accueil',
								}"
								class="hover:bg-primary-300 block text-center rounded-md px-3 py-2 text-base font-medium"
								>Accueil</RouterLink
							>
							<RouterLink
								to="/contact"
				...
				<div class="flex relative items-center max-w-md w-full">
					...
					<button
						id="search-button"
						class="border-none focus:outline-none absolute px-2 py-1.5 rounded-full right-3">
						<font-awesome-icon
				...
				<div class="-mr-2 flex sm:hidden">
					<button
						...
						<font-awesome-icon
							icon="fa-solid fa-bars"></font-awesome-icon>
					</button>
				</div>
			</div>
		</div>
		...
		<!-- Navigation mobile -->
			<Transition name="slide-left">
				<div v-if="isOpen" class="sm:hidden">
					<div
						class="absolute h-screen w-1/3 right-0 space-y-5 px-2 pb-3 pt-2 backdrop-blur shadow">
						<RouterLink
							to="/"
							:class="{
