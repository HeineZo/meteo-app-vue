...
const { weatherDataWeekly, selectedDay } = storeToRefs(weather);
...
<template>
    <main class="flex flex-col w-1/2 min-w-fit gap-5 my-10">
        <WeeklyWeather v-for="(weekday, index) in weatherDataWeekly" @click.native="selectedDay = index" :day="weekday.day"
